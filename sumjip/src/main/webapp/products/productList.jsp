...
<body>
	...
	<article class="prolist">
		...
		<table class="table">
			...
			<tr>
				<td><%=dto.getPno()%></td>
				<td><a href="ProductContent.pr?pno=<%=dto.getPno()%>"> <%=dto.getPname()%>
				</a></td>
				<td><%=dto.getPaddress()%></td>
				<td><%=dto.getPaddress2()%></td>
				<td><%=dto.getPprice()%></td>
				<td><%=dto.getPexplain()%></td>
			</tr>
			<% } %>
		</table>
		...
		%>
		<div id="table_search">
			<button type="button" class="btn btn-outline-success" onclick="location.href='ProductWriteForm.pr'">상품등록</button>
		</div>
		<%
		}
		}
		...
		<div class="room-pagination">
			...
			if (startPage > pageBlock) {
				%>
				<a href="ProductList.pr?pageNum=<%=startPage - pageBlock%>">[10페이지 이전] <i class="fa fa-long-arrow-right"></i>
				</a>
				<%
			}
			...
			if (endPage < pageCount) {
				%>
				<a href="ProductList.pr?pageNum=<%=startPage + pageBlock%>">[10페이지 다음] <i class="fa fa-long-arrow-right"></i>
				</a>
				<%
			}
			%>
