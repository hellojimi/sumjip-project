...
<body>
	...
	<div class="wrap">
		<article>
			...
			%>
			총 후기 개수 <%=reviewList.size()%> / 평균평점 <%=getAvgStar%>
			...
			<ul id="reviewList">
				...
				try {
					if (id.equals(writer)) {
					%>
					<li><button type="button" id="btns" class="btn btn-outline-success"
							onclick="location.href='MemberReviewModifyForm.me?ano=<%=rdto.getAno()%>'">후기 수정하기</button>
						<button type="button" id="btns" class="btn btn-outline-secondary"
							onclick="location.href='MemberReviewDelete.me?ano=<%=rdto.getAno()%>'">후기 삭제하기</button>
					</li>
					<%
					}
				} catch (Exception e) {
		...
		<div class="room-pagination">
			...
			%>
			<a href="ProductContent.pr?pageNum=<%=startPage - pageBlock%>&pno=<%=pno%>">[5페이지 이전]<i class="fa fa-long-arrow-right"></i>
			</a>
			<%
			}
			...
			%>
			<a href="ProductContent.pr?pageNum=<%=startPage + pageBlock%>&pno=<%=pno%>">[5페이지 다음]<i class="fa fa-long-arrow-right"></i>
			</a>
			<%
			}
			%>
